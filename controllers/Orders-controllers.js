...
const UpdateOrderStatus = (req, res) => {
  const sql =
    "UPDATE `orders` SET `OrderStatus` = '2' WHERE `orders`.`orderId` = ?";
...
const InsertCheckOutPage = (req, res) => {
  ...
  const sql =
    "INSERT INTO `checkoutpage` (`MemberId`,`UserName`, `City`, `district`, `mobile`, `address`, `email`, `recipientUserName`, `recipientCity`, `recipientDistrict`, `recipientMobile`, `recipientAddress`, `recipientEmail`) VALUES (?,?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)";
  db.query(sql, [
    req.body.data.Member,
    req.body.data.Name,
    req.body.data.City,
    req.body.data.District,
    req.body.data.Mobile,
    req.body.data.Address,
    req.body.data.Email,
    req.body.data.recipientName,
    req.body.data.recipientSelect,
    req.body.data.recipientDistrict,
    req.body.data.recipientMobile,
    req.body.data.recipientAddress,
    req.body.data.recipientEmail,
  ]).then(([r]) => {
    output.results = r;
    if (r.affectedRows && r.insertId) {
      output.success = true;
...
const additem = (req, res) => {
  ...
  const sql =
    "INSERT INTO `orderitemlist`(`MemberId`,`ItemName`, `ItemNamePrice`, `Total` ,`itemType`, `itemImg`, `itemQuantity`, `itemFlavor`, `itemCollection`, `itemEnglishName`, `itemEnglishFlavor`)  VALUES (?,?,?, ?, ?, ?, ?, ? , ?, ?, ?)";
  req.body.cartItems
    .forEach((element) => {
      db.query(sql, [
        element.MemberId,
...
const OrderCompeleted = async (req, res) => {
  ...
  db.query(sql, [req.params.orderId]).then(([r]) => res.json(r));
};

const address = async (req, res) => {
  const sql =
    "SELECT `orders`.`orderId`,`checkoutpage`.`UserName`,`checkoutpage`.`City`,`checkoutpage`.`district`,`checkoutpage`.`address`,`checkoutpage`.`recipientCity`,`checkoutpage`.`recipientDistrict`,`checkoutpage`.`recipientAddress`,`checkoutpage`.`mobile`,`checkoutpage`.`recipientMobile`,`checkoutpage`.`email`FROM  `orders` INNER JOIN `checkoutpage` WHERE `checkoutpage`.`create_time` = `orders`.`created_at`";
  db.query(sql, [req.params.orderId]).then(([r]) => res.json(r));
};
